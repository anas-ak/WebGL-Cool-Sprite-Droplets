...
class Application {
    constructor() {
        ...
        add('spriteBackground.jpg').
        add('spriteForeground.jpg').
        load(() => this.initialize());
    }

    /*
     * Initialize is run when the image loader is done loading all resources
     * @return void 
     */
    initialize() {
    ...
    /*
     * Simple resize function. Reinitializing everything on the canvas while changing the width
     * @return {void}
     */
    resizeCanvas() {
        this.width = window.innerWidth;
...
class EffectCanvas {
    constructor(width, height, loader) {
        ...
        shaderData.uniforms.uTextureBackground.value = loader.resources['spriteBackground.jpg'].texture;
        shaderData.uniforms.uTextureForeground.value = loader.resources['spriteForeground.jpg'].texture;
        shaderData.uniforms.vTextureSize.value = [
            loader.resources['spriteBackground.jpg'].texture.width,
            loader.resources['spriteBackground.jpg'].texture.height];
...
class DropletManager {
    constructor(stage, loader) {
        ...
        //Quick implementation to make sure there aren't extraordinary thunderstorms on mobile
        if (stage.width < 700) {
        ...
        // Define a position matrix so we can calculate all the edges of a droplet 
        // in a single loop
    ...
    */
    ...
    update(width, height) {
        ...
        // Trigger the spawn function for a small droplet as many times as is configured in the options
        for (let i = 0; i < this.options.spawnsPerFrame.small; i++) {
            this.spawnNewSmallDroplet(width, height);
        }

        // Trigger the spawn function for a large droplet as many times as is configured in the options
        ...
        // We don't need to process small droplets because they are dummy objects
        // that don't necessarily move after they've spawned
        this.checkLargeDropletLogic();
    }

    /*
     * Checks whether a big droplet hits the smaller droplet, 
     * if so, it grows by half of the smaller droplets size
     * @return {void}
     */
   ...
   */
   ...
   /*
    * Shrink a droplet based on the configured shrink speed.
    * If it will be too small, we set the mass to 0
  ...
  /*
   * Grow a droplet based on the targetMass  
   * @param {LargeDroplet} droplet
  */
  ...
  */
  checkDropletMovement(droplet) {
      // If the droplet is going to be removed at the end of this loop, 
